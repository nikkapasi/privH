...
static inline float getS(const float* S, int cols, int r, int c) {
    // Safe fetch for integral image with 0 for r<0 or c<0
...
static inline float rectSum(const float* S, int cols,
    ...
    // Inclusive rectangle [r1..r2, c1..c2] on integral image
    const float A = getS(S, cols, r2,   c2);
    const float B = getS(S, cols, r1-1, c2);
    const float C = getS(S, cols, r2,   c1-1);
    const float D = getS(S, cols, r1-1, c1-1);
    return A - B - C + D;
}
...
void ca_cfar_2d(
    ...
    float* scratch)
{
    // AXI & control (tool pragmas are fine; no ap_* types used)
...
#pragma HLS INTERFACE s_axilite port=return     bundle=control
    ...
    // ---- Pass 1: build integral image in 'scratch' ----
...
#pragma HLS PIPELINE II=1
            ...
            if (r1 >= 0 && c1 >= 0 && r2 < rows && c2 < cols && train_cells > 0) {
                ...
                const float noise_mu  = noise_sum / static_cast<float>(train_cells);
                const float thr       = k * noise_mu;
                const float cut       = rdm[idx];
                det = (cut > thr) ? 1u : 0u;
            } else {
                det = 0u; // edges: no decision
            }
